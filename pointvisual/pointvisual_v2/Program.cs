...
namespace pointvisual
{
    class Program
    {
        ...
        private static readonly string inputCsv = @"D:\projects\screencapture\screencapture\bin\Debug\project\targetTest\roma\test.csv";
        private static readonly string inputPic = @"D:\projects\screencapture\screencapture\bin\Debug\project\targetTest\roma\test.png";
        ...
        static void Main(string[] args)
        {
            ...
            image.Save(Path.Combine(output, "output_range.png"), ImageFormat.Png);
        ...
        public static List<Eye> GetLifeTGF() //TODO: пофиксить, чтобы было время фиксации взгляда (раскомментировать + доработать входные/выходные данные)!
        {
            ...
            foreach (var i in points)
            {
                foreach (var j in clusters)
                {
                    ...
                    {
                        output.Add(new Eye(eyeList[i.id].gazeX, eyeList[i.id].gazeY, j.timeFixation.ToString(), j.cluster));
                    }
                }
            }
            ...
            int sumY = 0;
            int counter = 0;
            List<Eye> rangeList = new List<Eye>();
            for (var i = 0; i < output.Count - 1; i++)
            {
                ...
                {
                    rangeList.Add(new Eye(sumX / counter, sumY / counter, output[i].timeStamp, output[i].cluster));
